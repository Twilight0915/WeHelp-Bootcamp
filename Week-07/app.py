...
def api_member():
    if request.method == "GET":
        ...
        # 對資料庫進行更新
        # 資料庫中 "name": "Tom"   => "name":  "湯米"
        sql_1 = "SELECT * FROM member WHERE name = %s"
        val_1 = [name]
        cursor.execute(sql_1, val_1)
        member_data = cursor.fetchone()
        # print(member_data)
        ...
        # 資料庫更新
        sql_2 = "UPDATE member SET name = %s WHERE name = %s"
        val_2 = (name_update, name)
        cursor.execute(sql_2, val_2)
        db.commit()
        ...
        # 更新成功，查詢的username姓名，更改為name_update
